	...
	public void compute_rhs()
	{
		...
		for (k = 1; k <= nz2; k++)
		{
			for (j = 1; j <= ny2; j++)
			{
				for (i = 1; i <= nx2; i++)
				{
					...
					rhs[1 ][ i ][ j ][ k ] = rhs[1 ][ i ][ j ][ k ] + dx2tx1 *
							  ...
							  tx2 * (u[1 ][ (i + 1) ][ j ][ k ] * up1 -
									 u[1 ][ (i - 1) ][ j ][ k ] * um1 +
									 (u[4 ][ (i + 1) ][ j ][ k ] -
											 square[(i + 1) + j * jsize2 + k * ksize2] - u[4 ][ (i - 1) ][ j ][ k ] + 
									  square[(i - 1) + j * jsize2 + k * ksize2]) * c2);

					rhs[2 ][ i ][ j ][ k ] = rhs[2 ][ i ][ j ][ k ] + dx3tx1 *
							  (u[2 ][ (i + 1) ][ j ][ k ] - 2.0 * u[2 ][ i ][ j ][ k ] +
							   u[2 ][ (i - 1) ][ j ][ k ]) + xxcon2 * (vs[i + 1][j][k] - 2.0 * vs[i ][ j ][ k ] +
										vs[i - 1][j][k]) -tx2 * (u[2 ][ (i + 1) ][ j ][ k ] * up1 - u[2 ][ (i - 1) ][ j ][ k ] * um1);
					...
					rhs[4 ][ i ][ j ][ k ] = rhs[4 ][ i ][ j ][ k ] + dx5tx1 *
							  ...
							  tx2 * ((c1 * u[4 ][ (i + 1) ][ j ][ k ] - c2 * square[i + 1][j][k]) * up1 -
									  (c1 * u[4 ][ (i - 1) ][ j ][ k ] - c2 * square[i - 1 + j * jsize2 + k * ksize2]) * um1);
				}
				...
				for (m = 0; m <= 4; m++)
				{
					rhs[m][i][j][k] = rhs[m][i][j][k] - dssp *(-4.0 * u[m ][ (i - 1) ][ j ][ k ] + 6.0 * u[m][i][j][k] -
		...
		for (k = 1; k <= nz2; k++)
		{
				...
				for (i = 1; i <= nx2; i++)
				{
					...
					rhs[4 ][ i ][ j ][ k ] = rhs[4 ][ i ][ j ][ k ] + dy5ty1 *
							 (u[4 ][ i ][ (j + 1) ][ k ] - 2.0 * u[4 ][ i ][ j ][ k ] +
							  u[4 ][ i ][ (j - 1) ][ k ]) + yycon3 * (qs[i ][ (j + 1) ][ k ] - 2.0 * qs[i ][ j ][ k ] +
									   qs[i ][ (j - 1) ][ k ]) +yycon4 * (vp1 * vp1 - 2.0 * vijk * vijk +vm1 * vm1) +
							 yycon5 * (u[4 ][ i ][ (j + 1) ][ k ] * rho_i[i ][ (j + 1) ][ k ] - 2.0 * u[4 ][ i ][ j ][ k ] * rho_i[i ][ j ][ k ] +
									   u[4 ][ i ][ (j - 1) ][ k ] * 
									   rho_i[i ][ (j - 1) ][ k ]) - ty2 * ((c1 * u[4 ][ i ][ (j + 1) ][ k ] -
			...
			for (i = 1; i <= nx2; i++)
			{
				for (m = 0; m <= 4; m++)
				{
					rhs[m][i][j][k] = rhs[m][i][j][k] - dssp * (5.0 * u[m][i][j][k] - 4.0 * u[m ][ i ][ (j + 1) ][ k ] +
									  u[m ][ i ][ (j + 2) ][ k ]);
				}
			}
			...
			for (i = 1; i <= nx2; i++)
			{
				for (m = 0; m <= 4; m++)
				{
					rhs[m][i][j][k] = rhs[m][i][j][k] - dssp *(-4.0 * u[m ][ i ][ (j - 1) ][ k ] + 6.0 * u[m][i][j][k] -
			...
			for (j = 3; j <= ny2 - 2; j++)
			{
				for (i = 1; i <= nx2; i++)
				{
					for (m = 0; m <= 4; m++)
					{
						rhs[m][i][j][k] = rhs[m][i][j][k] - dssp *(u[m ][ i ][ (j - 2) ][ k ] - 4.0 * u[m ][ i ][ (j - 1) ][ k ] +
			...
			for (i = 1; i <= nx2; i++)
			{
				for (m = 0; m <= 4; m++)
				{
					rhs[m][i][j][k] = rhs[m][i][j][k] - dssp * (u[m ][ i ][ (j - 2) ][ k ] - 4. * u[m ][ i ][ (j - 1) ][ k ] +
								5. * u[m][i][j][k]);
				}
			}
		}
				...
				for (i = 1; i <= nx2; i++)
				{
					...
					rhs[0 ][ i ][ j ][ k ] = rhs[0 ][ i ][ j ][ k ] + dz1tz1 *
							 (u[0 ][ i ][ j ][ (k + 1) ] - 2.0 * u[0 ][ i ][ j ][ k ] +
							  u[0 ][ i ][ j ][ (k - 1) ]) -tz2 * (u[3 ][ i ][ j ][ (k + 1) ] - u[3 ][ i ][ j ][ (k - 1) ]);
					rhs[1 ][ i ][ j ][ k ] = rhs[1 ][ i ][ j ][ k ] + dz2tz1 * (u[1 ][ i ][ j ][ (k + 1) ] - 2.0 * u[1 ][ i ][ j ][ k ] +
							  u[1 ][ i ][ j ][ (k - 1) ]) +zzcon2 * (us[i ][ j ][ (k + 1) ] - 2.0 * us[i ][ j ][ k ] +
									   us[i ][ j ][ (k - 1) ]) - tz2 * (u[1 ][ i ][ j ][ (k + 1) ] * wp1 -
									u[1 ][ i ][ j ][ (k - 1) ] * wm1);
					rhs[2 ][ i ][ j ][ k ] = rhs[2 ][ i ][ j ][ k ] + dz3tz1 * (u[2 ][ i ][ j ][ (k + 1) ] - 2.0 * u[2 ][ i ][ j ][ k ] +
							  u[2 ][ i ][ j ][ (k - 1) ]) + zzcon2 * (vs[i ][ j ][ (k + 1) ] - 2.0 * vs[i ][ j ][ k ] +
	...
	public void x_solve()
	{
		...
		for (k = 1; k <= nz2; k++)
		{
			for (j = 1; j <= ny2; j++)
			{
				...
				//               Scale the last row immediately
				//---------------------------------------------------------------------
				rhs[3 ][ i1 ][ j ][ k ] = rhs[3 ][ i1 ][ j ][ k ] /
				lhsp[2 ][ i1 ];
				rhs[4 ][ i1 ][ j ][ k ] = rhs[4 ][ i1 ][ j ][ k ] /
				lhsm[2 ][ i1 ];
