				...
				for (i = 1; i <= grid_points[0] - 2; i++) {
					...
					forcing[4][i][j][k] = forcing[4][i][j][k]
							...
							* (buf[ip1][1] * (c1 * ue[ip1][4] - c2 * q[ip1]) - buf[im1][1]
									* (c1 * ue[im1][4] - c2 * q[im1]))
							+ 0.5
							* xxcon3
							* (buf[ip1][0] - 2.0 * buf[i][0] + buf[im1 + 0
									* jsize3]) + xxcon4
							* (cuf[ip1] - 2.0 * cuf[i] + cuf[im1]) + xxcon5
		...
		for (k = 0; k <= grid_points[2] - 1; k++) {
			for (j = 0; j <= grid_points[1] - 1; j++) {
				for (i = 0; i <= grid_points[0] - 1; i++) {
					...
					us[i + j * jsize2 + k * ksize2] = u[1][i][j][k] * rho_inv;
					vs[i + j * jsize2 + k * ksize2] = u[2][i][j][k] * rho_inv;
					ws[i + j * jsize2 + k * ksize2] = u[3][i][j][k] * rho_inv;
					...
					rhs[4][i][j][k] = rhs[4][i][j][k]
							...
							* (u[4][(i + 1)][j][k]
									...
									* u[4 + i * isize1 + j * jsize1 + k
											* ksize1]
									* rho_i[i + j * jsize2 + k * ksize2] + u[4][(i - 1)][j][k]
									* rho_i[i - 1 + j * jsize2 + k * ksize2])
							- tx2
							* ((c1 * u[4][(i + 1)][j][k] - c2
									* square[i + 1 + j * jsize2 + k * ksize2])
