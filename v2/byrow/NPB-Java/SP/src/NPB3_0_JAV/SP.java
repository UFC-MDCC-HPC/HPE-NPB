...
//			else adi();
	...
	{
		int niter = 0;
		File file = new File ("inputsp.data");
        ...
        if (file.exists())
        {
			try
			{
				...
				niter = Integer.valueOf(sss.readLine());
				dt = Integer.valueOf(sss.readLine());
				grid_points[0] = Integer.valueOf(sss.readLine());
                grid_points[1] = Integer.valueOf(sss.readLine());
                grid_points[2] = Integer.valueOf(sss.readLine());
	//			fis.close();
			}
			catch (Exception e)
			{
	...
	public void printTimers(String[] t_names, double[] trecs, double tmax)
	{
		...
		for (int i = 1; i < t_last; i++)
		{
			...
			System.out.println(t_names[i] + ":" + trecs[i] +
						   "  (" + dbl + "%)");
			if (i == t_rhs)
			{
				t = trecs[t_rhsx] + trecs[t_rhsy] + trecs[t_rhsz];
                dbl = (t * 100.0 / tmax);
				System.out.println("    --> total " + "sub-rhs" + ":" + (t) +
							   "  (" + dbl + "%)");
				t = trecs[t_rhs] - t;
                dbl = (t * 100.0 / tmax);
				System.out.println("    --> total " + "rest-rhs" + ":" + (t) +
							   "  (" + dbl + "%)");
			}
			else if (i == t_zsolve)
			{
                ...
                dbl = (t * 100.0 / tmax);
				System.out.println("    --> total " + "sub-zsol" + ":" + (t) +
							   "  (" + dbl+ "%)");
			}
			else if (i == t_rdis2)
			{
                ...
                dbl = (t * 100.0 / tmax);
				System.out.println("    --> total " + "redist" + ":" + (t) +
							   "  (" + dbl + "%)");
			}
		}
	}
