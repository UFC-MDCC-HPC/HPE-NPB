...
namespace NPB {
    public class FT: FTBase {
        ...
        public void vranlc(int n, double x, double a, double[] u1, int k) {
            ...
            for(j = 0; j <= n - 1; j = j + nv) {
                ...
                for(i = 0; i < n1; i++) { //y(i+j) = r46 * xv(i)
                    idx = (i+j)+(2*dims[0, 0]*dims[1, 0]*k);//idx = (i+j)+(2*nx*dims[1, 0]*k);
                    //y[idx] = r46 * xv[i - 1];
                    u1[idx] = r46*xv[i];  //u0[d3, d2, d1]
                }
                //c---------------------------------------------------------------------
                ...
                for(jj = 0; jj <= (d2-fftblock); jj = jj + fftblock) {
                    ...
                    for(j = 0; j < fftblock; j++) {
                        for(i = 0; i < d1; i++) {//y(j,i,1) = x(i,j+jj,k)
                            io = ((k*d2+(j+jj))*d1+i)*2;
                            y[0, i, j, REAL] = x[io+REAL]; //y[1,i,j,real] = x[k,j+jj,i,real]
                            y[0, i, j, IMAG] = x[io+IMAG];
                        }
                    }
                    if(timers_enabled)
                    ...
                    for(j = 0; j < fftblock; j++) {
                        for(i = 0; i < d1; i++) {
                            ...
                            io  = (((k*d2+(j+jj))*d1+i)*2);
                            xout[io+REAL] = y[0,i,j,REAL];
                            xout[io+IMAG] = y[0,i,j,IMAG];
                        }
                    }
                    if(timers_enabled)
                        timer.stop(T_fftcopy);
                ...
                for(ii = 0; ii <= d1 - fftblock; ii = ii + fftblock) {
                    ...
                    for(j = 0; j < d2; j++) {
                        for(i = 0; i < fftblock; i++) {
                            ...
                            y[0, j, i, REAL] = x[iin+REAL];
                            y[0, j, i, IMAG] = x[iin+IMAG];
                        }
                    }
                    if(timers_enabled)
                        timer.stop(T_fftcopy);
                    ...
                    for(j = 0; j < d2; j++) {
                        for(i = 0; i < fftblock; i++) {
                            ...
                            io = ((k * d2 + j) * d1 + (i + ii)) * 2;
                            xout[io+REAL] = y[0,j,i,REAL];
                            xout[io+IMAG] = y[0,j,i,IMAG];
                        }
                    }
                    if(timers_enabled)
                        timer.stop(T_fftcopy);
            ...
            for(j = 0; j < d2; j++) {
                for(ii = 0; ii <= d1 - fftblock; ii = ii + fftblock) {
                    ...
                    for(k = 0; k < d3; k++) {
                        for(i = 0; i < fftblock; i++) {
                            iin = ((k*d2+j)*d1+(i+ii))*2;
                            y[0, k, i, REAL] = x[iin+REAL];
                            y[0, k, i, IMAG] = x[iin+IMAG];
                        }
                    }
                    if(timers_enabled)
                        timer.stop(T_fftcopy);
                    ...
                    for(k = 0; k < d3; k++) {
                        for(i = 0; i < fftblock; i++) {
                            ...
                            io  = (((k*d2+j)*d1+(i+ii))*2);
                            xout[io+REAL] = y[0,k,i,REAL];
                            xout[io+IMAG] = y[0,k,i,IMAG];
                        }
                    }
                    if(timers_enabled)
                        timer.stop(T_fftcopy);
            ...
            for(k = 0; k < d3; k++) { //k=1 <= d3        
                for(i = 0; i < d1; i++) { //i=1 <= d1 
                    for(j = 0; j < d2; j++) { // j=1 <= d2                     //xout(j,k,i)    =    xin(i,j,k); 
                        ...
                        uxout[io+REAL] = uxin[ii+REAL];
                        uxout[io+IMAG] = uxin[ii+IMAG];
                    }
                }
            }
            if(timers_enabled)
        ...
        public void transpose_x_y_global(int d1, int d2, int d3, double[] uxin, double[] uxout) {
            ...
            commslice2.AlltoallFlattened<double>(uxin, d1*d2*d3*2/np1, ref uxout);
                ...
                for(k = 0; k < d3; k++) {
                    for(j = 0; j < d2; j++) {
                        for(i = 0; i < (d1/np1); i++) { //io = ((((k-1)*size3+(j-1))*size4*np1+(i+ioff-1))) *2;  //uxout[k,j,i+ioff] ii  = ((((p)*size2+(j-1))*size4+(k-1))*size3+(i-1))*2; //uxin[p,j,k,i]
                            ...
                            uxout[io+REAL] = uxin[ii+REAL];    //xout(i+ioff,j,k) = xin(i,k,j,p);
                            uxout[io+IMAG] = uxin[ii+IMAG];
                        }
                    }
                }
                ...
                if(n1 >= n2) {
                    for(j = 0; j < n2; j++) {
                        for(i = 0; i < n1; i++) {
                            ...
                            iin = (j * n1 + i)*2;
                            io = (i * n2 + j)*2;
                            uxout[io+REAL] = uxin[iin+REAL];
                            uxout[io+IMAG] = uxin[iin+IMAG];
                        }
                    }
                }
                else {
                    ...
                    for(i = 0; i <= n1 - 1; i = i + transblock) {
                        ...
                        for(jj = 0; jj < transblock; jj++) {
                            for(ii = 0; ii < transblock; ii++) { //z(jj,ii) = xin(i+ii, j+jj);
                                ...
                                z[ii,jj,REAL] = uxin[iin + REAL];
                                z[ii,jj,IMAG] = uxin[iin + IMAG];
                            }
                        }
                        for(ii = 0; ii < transblock; ii++) {
                            for(jj = 0; jj < transblock; jj++) {//xout(j+jj, i+ii) = z(jj,ii);
                                ...
                                uxout[io + REAL] = z[ii, jj, REAL];
                                uxout[io + IMAG] = z[ii, jj, IMAG];
        ...
        public void transpose2_global(double[] uxin, double[] uxout) {
            ...
            //Point.setVetor(uxin, src);
            commslice1.AlltoallFlattened<double>(uxin, ntdivnp * 2 / np, ref uxout);
            //Point.setVetor(dst, uxout);
                    ...
                    for(ii = 0; ii <= d1 - block1; ii = ii + block1) {
                        for(k = 0; k < block3; k++) {
                            ...
                            for(i = 0; i < block1; i++) {                                 //buf(k, i) = xin(i+ii, j, k1);
                                ...
                                buf[i,k,REAL] = xin[iin + REAL];
                                buf[i,k,IMAG] = xin[iin + IMAG];
                            }
                        }
                        for(i = 0; i < block1; i++) {
                            i1 = i + ii;
                            for(k = 0; k < block3; k++) {                                 //xout(k+kk, j, i1) = buf(k, i);
                                ...
                                xout[io + REAL] = buf[i,k,REAL];
                                xout[io + IMAG] = buf[i,k,IMAG];
                            }
                        }
                    }
                }
            ...
            for(j = 0; j < d2; j++) {
                for(k = 0; k < d3; k++) {
                    for(i = 0; i < d1; i++) {                                             //xout(k, j, i) = xin(i, j, k);
                        ...
                        xout[io + REAL] = xin[iin + REAL];
                        xout[io + IMAG] = xin[iin + IMAG];
                    }
                }
            }
        //---------------------------------------------------------------------
        ...
        public void transpose_x_z_global(int d1, int d2, int d3, double[] xin, double[] xout) {
            ...
            commslice1.AlltoallFlattened<double>(xin, d1*d2*d3*2/np2, ref xout);
                ...
                if(q >= xstart[0] && q <= xend[0]) {
                    ...
                    if(r >= ystart[0] && r <= yend[0]) {
                        ...
                        if(s >= zstart[0] && s <= zend[0]) {               //chk=chk+u11(q-xstart(1)+1,r-ystart(1)+1,s-zstart(1)+1);
                            ...
                            chk_Real=chk_Real+u2[idx+REAL]; //u11[i1, i2, i3];
                            chk_Imag=chk_Imag+u2[idx+IMAG]; //u11[i1, i2, i3];
                        }
                    }
                }
            }
        ...
        public int verify(int d1, int d2, int d3, int nt, double[] sums) {
            ...
            if(d1 == 64 && d2 == 64 && d3 == 64 && nt == 6) {
                ...
                csum_ref[2, REAL] = 554.6148406171;
                csum_ref[3, REAL] = 554.5423607415;
                csum_ref[4, REAL] = 554.4255039624;
                csum_ref[5, REAL] = 554.2683411902;

                csum_ref[0, IMAG] = 484.5363331978;
                csum_ref[1, IMAG] = 486.5304269511;
                csum_ref[2, IMAG] = 488.3910722336;
                csum_ref[3, IMAG] = 490.1273169046;
                csum_ref[4, IMAG] = 491.7475857993;
                ...
                csum_ref[2, REAL] = 559.4024089970;
                csum_ref[3, REAL] = 556.0698047020;
                csum_ref[4, REAL] = 553.0898991250;
                csum_ref[5, REAL] = 550.4159734538;

                csum_ref[0, IMAG] = 529.3246849175;
                csum_ref[1, IMAG] = 528.2149986629;
                csum_ref[2, IMAG] = 527.0996558037;
                csum_ref[3, IMAG] = 526.0027904925;
                csum_ref[4, IMAG] = 524.9400845633;
                ...
                csum_ref[2, REAL] = 506.9376896287;
                csum_ref[3, REAL] = 507.7892868474;
                csum_ref[4, REAL] = 508.5233095391;
                csum_ref[5, REAL] = 509.1487099959;

                csum_ref[0, IMAG] = 511.4047905510;
                csum_ref[1, IMAG] = 509.8809666433;
                csum_ref[2, IMAG] = 509.8144042213;
                csum_ref[3, IMAG] = 510.1336130759;
                ...
                csum_ref[2, REAL] = 514.6409228649;
                csum_ref[3, REAL] = 514.2378756213;
                csum_ref[4, REAL] = 513.9626667737;
                csum_ref[5, REAL] = 513.7423460082;
                csum_ref[6, REAL] = 513.5547056878;
                csum_ref[7, REAL] = 513.3910925466;
                csum_ref[8, REAL] = 513.2470705390;
                csum_ref[9, REAL] = 513.1197729984;
                csum_ref[10, REAL] = 513.0070319283;
                csum_ref[11, REAL] = 512.9070537032;
                csum_ref[12, REAL] = 512.8182883502;
                csum_ref[13, REAL] = 512.7393733383;
                csum_ref[14, REAL] = 512.6691062020;
                csum_ref[15, REAL] = 512.6064276004;
                csum_ref[16, REAL] = 512.5504076570;
                csum_ref[17, REAL] = 512.5002331720;
                csum_ref[18, REAL] = 512.4551951846;
                csum_ref[19, REAL] = 512.4146770029;

                csum_ref[0, IMAG] = 507.7803458597;
                csum_ref[1, IMAG] = 508.8249431599;
                csum_ref[2, IMAG] = 509.6208912659;
                csum_ref[3, IMAG] = 510.1023387619;
                csum_ref[4, IMAG] = 510.3976610617;
                csum_ref[5, IMAG] = 510.5948019802;
                csum_ref[6, IMAG] = 510.7404165783;
                csum_ref[7, IMAG] = 510.8576573661;
                csum_ref[8, IMAG] = 510.9577278523;
                csum_ref[9, IMAG] = 511.0460304483;
                csum_ref[10, IMAG] = 511.1252433800;
                csum_ref[11, IMAG] = 511.1968077718;
                csum_ref[12, IMAG] = 511.2616233064;
                csum_ref[13, IMAG] = 511.3203605551;
                csum_ref[14, IMAG] = 511.3735928093;
                csum_ref[15, IMAG] = 511.4218460548;
                csum_ref[16, IMAG] = 511.4656139760;
                csum_ref[17, IMAG] = 511.5053595966;
                ...
                csum_ref[2, REAL] = 514.4678022222;
                csum_ref[3, REAL] = 514.0150594328;
                csum_ref[4, REAL] = 513.7550426810;
                csum_ref[5, REAL] = 513.5811056728;
                csum_ref[6, REAL] = 513.4569343165;
                csum_ref[7, REAL] = 513.3651975661;
                csum_ref[8, REAL] = 513.2955192805;
                csum_ref[9, REAL] = 513.2410471738;
                csum_ref[10, REAL] = 513.1971141679;
                csum_ref[11, REAL] = 513.1605205716;
                csum_ref[12, REAL] = 513.1290734194;
                csum_ref[13, REAL] = 513.1012720314;
                csum_ref[14, REAL] = 513.0760908195;
                csum_ref[15, REAL] = 513.0528295923;
                csum_ref[16, REAL] = 513.0310107773;
                csum_ref[17, REAL] = 513.0103090133;
                csum_ref[18, REAL] = 512.9905029333;
                csum_ref[19, REAL] = 512.9714421109;

                csum_ref[0, IMAG] = 514.9019699238;
                csum_ref[1, IMAG] = 512.7578201997;
                csum_ref[2, IMAG] = 512.2251847514;
                csum_ref[3, IMAG] = 512.1090289018;
                csum_ref[4, IMAG] = 512.1143685824;
                csum_ref[5, IMAG] = 512.1496764568;
                csum_ref[6, IMAG] = 512.1870921893;
                csum_ref[7, IMAG] = 512.2193250322;
                csum_ref[8, IMAG] = 512.2454735794;
                csum_ref[9, IMAG] = 512.2663649603;
                csum_ref[10, IMAG] = 512.2830879827;
                csum_ref[11, IMAG] = 512.2965869718;
                csum_ref[12, IMAG] = 512.3075927445;
                csum_ref[13, IMAG] = 512.3166486553;
                csum_ref[14, IMAG] = 512.3241541685;
                csum_ref[15, IMAG] = 512.3304037599;
                csum_ref[16, IMAG] = 512.3356167976;
                csum_ref[17, IMAG] = 512.3399592211;
                ...
                csum_ref[2, REAL] = (511.9865766760);
                csum_ref[3, REAL] = (511.9518799488);
                csum_ref[4, REAL] = (511.9269088223);
                csum_ref[5, REAL] = (511.9082416858);
                csum_ref[6, REAL] = (511.8943814638);
                ...
                csum_ref[2, IMAG] = (511.7096364601);
                csum_ref[3, IMAG] = (511.7373863950);
                csum_ref[4, IMAG] = (511.7680347632);
                csum_ref[5, IMAG] = (511.7967875532);
                csum_ref[6, IMAG] = (511.8225281841);
                ...
                csum_ref[2, REAL] = 512.0623229306;
                csum_ref[3, REAL] = 512.0438418997;
                csum_ref[4, REAL] = 512.0311521872;
                csum_ref[5, REAL] = 512.0226088809;
                csum_ref[6, REAL] = 512.0169296534;
                ...
                csum_ref[2, IMAG] = 511.9074203747;
                csum_ref[3, IMAG] = 511.9345900733;
                csum_ref[4, IMAG] = 511.9551325550;
                csum_ref[5, IMAG] = 511.9720179919;
                csum_ref[6, IMAG] = 511.9861371665;
