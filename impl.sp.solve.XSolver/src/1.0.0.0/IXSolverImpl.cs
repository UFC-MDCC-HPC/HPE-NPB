...
namespace impl.sp.solve.XSolver { 
		...
		where DIR:IX
	{
		...
		{		
		    ...
		    int c, i, j, k, n, iend, jsize, ksize, i1, m, p, istart, stage;
		    ...
		    for (stage = 0; stage < ncells; stage++)
		    {
		        ...
		        Input_buffer_forward.Array = in_buffer_x = in_buffer_solver[0,stage];// new double[22*buffer_size];
		        Output_buffer_forward.Array = out_buffer_x = out_buffer_solver[0,stage]; // new double[22*buffer_size];
		    ...
		    for (stage = ncells - 1; stage >= 0; stage--)
		    {
		       ...
		       // buffer_size = (jsize - start[c, 1] - end[c, 1]) * (ksize - start[c, 2] - end[c, 2]);
		        ...
		        in_buffer_x = Input_buffer_backward.Array = in_buffer_solver[1,stage]; //new double[10 * buffer_size];
		        out_buffer_x = Output_buffer_backward.Array = out_buffer_solver[1,stage]; //new double[10 * buffer_size];
