...
namespace NPB {
    public class LU: LUBase {
            ...
            if(k>0) {
                ...
                if(iex == 0) {
                    if(north != -1) {
                        ...
                        int idx = 0;
                        mid = worldcomm.ImmediateReceive<double>(north, from_n, dum1); //call MPI_RECV[ dum1[1,jst],5*[jend-jst+1],dp_type,north,from_n,MPI_COMM_WORLD,status,IERROR ]
                        mid.Wait();
                        for(j=jst; j<=jend; j++) {
                    ...
                    if(west != -1) {
                        ...
                        int idx = 0;
                        mid = worldcomm.ImmediateReceive<double>(west, from_w, dum1); //call MPI_RECV[ dum1[1,ist],5*[iend-ist+1],dp_type,west,from_w,MPI_COMM_WORLD,status,IERROR ]
                        mid.Wait();
                        for(i=ist; i<=iend; i++) {
                    ...
                    if(south != -1) {
                        double[] dum1 = new double[(5*(jend-jst+1))];
                        int idx = 0;
                        mid = worldcomm.ImmediateReceive<double>(south, from_s, dum1); //call MPI_RECV[ dum1[1,jst],5*[jend-jst+1],dp_type,south,from_s,MPI_COMM_WORLD,status,IERROR ]
                        mid.Wait();
                        for(j=jst; j<=jend; j++) {
                    ...
                    if(east != -1) {
                        double[] dum1 = new double[(5*(iend-ist+1))];
                        int idx = 0;
                        mid = worldcomm.ImmediateReceive<double>(east, from_e, dum1); //call MPI_RECV[ dum1[1,ist],5*[iend-ist+1],dp_type,east,from_e,MPI_COMM_WORLD,status,IERROR ]
                        mid.Wait();
                        for(i=ist; i<=iend; i++) {
                    ...
                    if(south != -1) {
                        ...
                        worldcomm.Send<double>(dum, south, from_n); //call MPI_SEND[ dum[1,jst], 5*[jend-jst+1], dp_type, south, from_n, MPI_COMM_WORLD, IERROR ]
                    }
                    if(east != -1) {
                        ...
                        worldcomm.Send<double>(dum, east, from_w); //call MPI_SEND[ dum[1,ist], 5*[iend-ist+1], dp_type, east, from_w, MPI_COMM_WORLD, IERROR ]
                    }
                }
                else {
                    if(north != -1) {
                        ...
                        worldcomm.Send<double>(dum, north, from_s); //call MPI_SEND[ dum[1,jst], 5*[jend-jst+1], dp_type, north, from_s, MPI_COMM_WORLD, IERROR ]
                    }
                    if(west != -1) {
                        double[] dum = new double[(5*(iend-ist+1))];
                        int idx = 0;
                        ...
                        worldcomm.Send<double>(dum, west, from_e); //call MPI_SEND[ dum[1,ist], 5*[iend-ist+1], dp_type, west, from_e, MPI_COMM_WORLD, IERROR ]
                    }
                }
            }
            else {
        ...
        public double pintgr() {
            ...
            //---------------------------------------------------------------------
            exchange4(phi1, phi2, ibeg, ifin1, jbeg, jfin1); 
        ...
        //exchange_4.f
        public void exchange4(double[,] g, double[,] h, int ibeg, int ifin1, int jbeg, int jfin1) {
            //---------------------------------------------------------------------
            //   compute the right hand side based on exact solution
            //---------------------------------------------------------------------
            //  input parameters
