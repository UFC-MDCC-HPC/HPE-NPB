...
namespace NPB3_0_JAV.FTThreads {
    ...
    public class FFTThread  //: Thread
    {
        ...
        /* comentado ate resolver threading
    ...
    /* codigo traduzido
      lock(this){
        while(done==true){
          try{
	        wait();
            lock(master){ master.notify();}
          }catch(InterruptedException ie){}
        }
        step();
        state++;
        if(state==4) state=1;
        lock(master){done=true;master.notify();}
      }
     fim */
    }
  }

        public void setVariables(int sign1, bool tr, double[] x1,
        ...
        /* Comentado ate correcao de thread
        public void step()
        {
            int log = ilog2(n2);
            int isize3 = 2,
