        ...
        */
        ...
        public void CalculateChecksum(double[] csum, int csmffst, int iterN, double[,,,] u, int d1,int d2, int d3)
        {
            ...
            int isize3 = 2,
                jsize3 = isize3 * (d1 + 1),
                ksize3 = jsize3 * d2;
            csum[csmffst+REAL] = 0.0;  //  csum[REAL + csmffst] = 0.0;
            csum[csmffst+IMAG] = 0.0;  //  csum[IMAG + csmffst] = 0.0;
            ...
            csum[csmffst+REAL] = csumr / (d1 * d2 * d3);  //  csum[REAL + csmffst] = csumr / (d1 * d2 * d3);
            csum[csmffst+IMAG] = csumi / (d1 * d2 * d3);  //  csum[IMAG + csmffst] = csumi / (d1 * d2 * d3);
        ...
        public int verify(int ires, int n1, int n2, int n3, int nt, double[] cksum)
        {
            ...
            else
            {
                ...
                {
                    double csumr = (cksum[(it*2)+REAL] - cexpd[REAL + it * 2]) / cexpd[REAL + it * 2];  //  double csumr = (cksum[REAL + it * 2] - cexpd[REAL + it * 2]) / cexpd[REAL + it * 2];
                    double csumi = (cksum[(it*2)+IMAG] - cexpd[IMAG + it * 2]) / cexpd[IMAG + it * 2];  //  double csumi = (cksum[IMAG + it * 2] - cexpd[IMAG + it * 2]) / cexpd[IMAG + it * 2];
